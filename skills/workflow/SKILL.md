...
티켓 프로젝트 키로 서브 레포를 자동 탐색. 경로가 명확하지 않은 경우 항상 실행.

```bash
PROJECT_KEY="{KM}"  # 티켓 키에서 추출

# 1. 워크트리 제외, 메인 레포만 추출
# 2. remote URL에 프로젝트 키 포함 여부로 매칭
for dir in ../*/; do
  dirname=$(basename "$dir")
  is_worktree=$(git -C "$dir" rev-parse --git-dir 2>/dev/null | grep -c "worktrees")
  if [ "$is_worktree" = "0" ] && git -C "$dir" rev-parse --git-dir > /dev/null 2>&1; then
...
# → 후보 1개: 자동 확정
# → 후보 여러 개: 사용자에게 선택 요청
# → 후보 없음: 사용자에게 레포명 확인 후 git clone
```
